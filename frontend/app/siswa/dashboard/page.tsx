...
export default function SiswaDashboardPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Row 2: Statistik Absensi (col-4) + Progress Tugas + Jadwal (col-8) ─ */}
      <div className="grid grid-cols-12 gap-4 md:gap-5">

        {/* Rainbow radialBar — distribusi absensi siswa */}
        <motion.div
        ...
        >
          ...
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">Rekap kehadiran kamu semester ini</p>
          <StatisticRainbow
            hadir={absensi.hadir}
            sakit={0}
            izin={absensi.izin}
        ...
        {/* Progress Tugas + Jadwal Hari Ini — col-8 */}
        <div className="col-span-12 flex flex-col gap-4 xl:col-span-8">
          ...
          <motion.div
            ...
            className="flex items-center gap-6 rounded-2xl bg-white p-6 shadow-[0_2px_8px_rgba(0,0,0,0.07)] dark:bg-[#1c2434]"
          >
            {/* Donut */}
            ...
            <div className="flex-1">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <p className="text-base font-bold text-slate-800 dark:text-white">Progress Tugas</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {tugasProgress.dikumpulkan} dari {tugasProgress.total} tugas dikumpulkan
                  </p>
                </div>
              ...
              <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
                <motion.div className="h-full rounded-full"
                  style={{ backgroundColor: P }}
                  initial={{ width: 0 }}
                  animate={{ width: `${tugasPersen}%` }}
                  transition={{ duration: 1.1, delay: 0.4, ease: [0.16, 1, 0.3, 1] }} />
              </div>
              <div className="mt-2 flex items-center gap-3">
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full" style={{ backgroundColor: P }} />
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">
                    Selesai <strong className="text-slate-700 dark:text-slate-200">{tugasProgress.dikumpulkan}</strong>
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full" style={{ backgroundColor: R }} />
                  <span className="text-[11px] text-slate-500 dark:text-slate-400">
                    Belum <strong className="text-slate-700 dark:text-slate-200">{tugasBelum}</strong>
                  </span>
                </div>
              </div>
            </div>
          ...
          <SectionCard title="Jadwal Hari Ini" icon={Calendar} iconColor={R}
            ...
            delay={0.35} className="flex-1" bodyClass="px-4 py-2">
            {data.jadwalHariIni.length === 0 ? (
              <p className="py-6 text-center text-sm text-slate-500 dark:text-slate-400">Tidak ada jadwal hari ini</p>
            ) : (
                ...
                {data.jadwalHariIni.map((j, i) => {
                  ...
                  return (
                    <motion.li key={j.id}
                      ...
                      className={`flex items-center gap-3 rounded-xl px-2 py-2.5 transition-colors ${live ? "bg-blue-50 dark:bg-blue-900/20" : "hover:bg-slate-50 dark:hover:bg-slate-700/30"}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400">
                          {j.guru?.user?.nama ?? "–"} · {j.jamMulai}–{j.jamSelesai}
                        </p>
                      </div>
                      {live && (
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {CARDS.map((card, i) => (
          <motion.div key={card.href}
            ...
            transition={{ duration: 0.35, delay: 0.45 + i * 0.07, ease: [0.16, 1, 0.3, 1] }}
          >
            <Link href={card.href}
      ...
      {/* ── Row 4: Tugas Deadline (col-5) + Pengumuman (col-7) ──────────────── */}
      <div className="grid grid-cols-12 gap-4 md:gap-5">
