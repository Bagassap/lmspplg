...
import {
  Search, Clock, LogIn, ClipboardCheck,
  BookOpen, ChevronDown,
  Calendar, GraduationCap, CalendarDays, BookMarked,
  School, MapPin, RefreshCw, X, ChevronRight,
...
function ActionBtn({
  ...
  return (
    <motion.button onClick={onClick} title={title}
      ...
      className="flex flex-1 items-center justify-center gap-1.5 rounded-xl px-2 py-2.5 text-[11px] font-bold transition-all hover:brightness-95 active:scale-95"
...
export default function GuruJadwalKelasPage() {
  ...
  const totalMapel = new Set(jadwal.map((j) => j.mataPelajaran)).size;
  ...
  const filtered = jadwal
    .filter((j) => {
      ...
      if (search) {
        ...
        if (!j.mataPelajaran.toLowerCase().includes(q) && !j.kelas.toLowerCase().includes(q)) return false;
      }
      return true;
    })
    .sort((a, b) => {
  ...
  const fetchJadwal = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/jadwal-kelas/saya");
  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="mb-2.5 flex items-center gap-1.5 text-[11px] font-medium text-white/60">
              ...
              <span className="text-white/90">Jadwal Mengajar</span>
            </div>
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h1 className="text-2xl font-extrabold leading-tight text-white md:text-3xl">Jadwal Mengajar</h1>
                <p className="mt-0.5 text-sm text-white/70">Kelas-kelas yang Anda ampu</p>
              </div>
            </div>
          </div>
          <LiveClock />
        ...
        <div className="relative mt-5 flex flex-wrap gap-2">
          {[
            ...
            { icon: GraduationCap, label: `${loading ? "—" : allKelas.length} Kelas` },
            { icon: BookMarked,    label: `${loading ? "—" : totalMapel} Mapel` },
            { icon: Clock,         label: `${loading ? "—" : jadwalHariIni} Hari Ini` },
          ].map(({ icon: Ic, label }) => (
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <CreditStatCard icon={Calendar}      label="Jadwal Saya"    value={loading ? "—" : jadwal.length}   suffix="jadwal" sub="Total aktif"   gradient="linear-gradient(135deg,#3B7CE8,#4F8EF7)" loading={loading} delay={0.05} />
        <CreditStatCard icon={GraduationCap} label="Total Kelas"    value={loading ? "—" : allKelas.length} suffix="kelas"  sub="Kelas diampu"  gradient="linear-gradient(135deg,#EF4444,#F87171)" loading={loading} delay={0.10} />
        <CreditStatCard icon={BookMarked}    label="Mapel Berbeda"  value={loading ? "—" : totalMapel}      suffix="mapel"  sub={`${Math.ceil(jadwal.length / Math.max(allKelas.length, 1))} jadwal/kelas`} gradient="linear-gradient(135deg,#F59E0B,#FCD34D)" loading={loading} delay={0.15} />
        <CreditStatCard icon={Users}         label="Jadwal Hari Ini" value={loading ? "—" : jadwalHariIni} suffix="jadwal" sub={`Aktif hari ${today}`} gradient="linear-gradient(135deg,#10B981,#34D399)" loading={loading} delay={0.20} />
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-100 bg-white py-24 text-center dark:border-slate-700/50 dark:bg-[#1c2434]">
            ...
            <p className="font-bold text-slate-700 dark:text-slate-300">
              {hasActiveFilter ? "Tidak ada jadwal ditemukan" : "Belum ada jadwal mengajar"}
            </p>
            <p className="mt-1 text-sm text-slate-400 dark:text-slate-500">
              {hasActiveFilter ? "Coba ubah kata kunci atau filter" : "Hubungi admin untuk mengatur jadwal Anda"}
            </p>
            {hasActiveFilter && (
              <motion.button
                onClick={() => { setFilterHari(todayDefault); setFilterKelas(""); setSearch(""); }}
                whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
                className="mt-6 flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-2 text-sm font-semibold text-slate-600 shadow-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-700/50 dark:text-slate-300 dark:hover:bg-slate-700">
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-2.5 px-4 py-3.5 xl:hidden">
                  <div className="flex items-center gap-3">
                    {IconBox}
                    <p className="min-w-0 flex-1 truncate text-[15px] font-bold text-slate-900 dark:text-white">{item.mataPelajaran}</p>
                  </div>
                  {item.ruangan && (
                  ...
                  <div className="flex items-center gap-1.5">
                    <ActionBtn title="Masuk ke kelas" label="Masuk" icon={LogIn} color="#4F8EF7" colorBg="#EEF4FF"
                      onClick={() => router.push(`/guru/jadwal-kelas/${item.slug ?? item.id}`)} />
                    <ActionBtn title="Input presensi" label="Presensi" icon={ClipboardCheck} color="#EF4444" colorBg="#FFF1F2"
                      onClick={() => setPresensi({ jadwalKelasId: item.id, kelas: item.kelas })} />
                  </div>
                </div>
