...
export async function GET(request: Request) {
  ...
  const saya = searchParams.get("saya");
  const kelas = searchParams.get("kelas");

  const path = saya === "1" ? "/api/jadwal-kelas/saya" : "/api/jadwal-kelas";
  const qs = kelas ? `?kelas=${encodeURIComponent(kelas)}` : "";

  try {
    const res = await fetch(`${BACKEND}${path}${qs}`, {
...
export async function POST(request: Request) {
  ...
  try {
    const res = await fetch(`${BACKEND}/api/jadwal-kelas`, {
      method: "POST",
