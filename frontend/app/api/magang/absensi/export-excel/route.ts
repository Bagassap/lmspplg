...
export async function GET(request: Request) {
  ...
  for (const key of ["tempatMagangId", "tanggal", "mode", "tanggalMulai", "tanggalSelesai", "bulan", "tahun"]) {
  ...
  try {
    backendRes = await fetch(`${BACKEND}/api/magang/absensi/export-excel?${qs}`, {
  ...
  headers.set("Content-Disposition", disposition ?? "attachment; filename=\"AbsensiPKL.xlsx\"");
