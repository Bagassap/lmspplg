        ...
        <div id="status-kehadiran-hari-ini" className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="flex h-full flex-col rounded-3xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 lg:col-span-2">
            ...
            <div className="mt-4 border-t border-slate-100 pt-4 dark:border-slate-700">
              <p className="mb-2 text-[11px] font-semibold text-slate-400">Ringkasan Kehadiran</p>
              <div className="flex h-2.5 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
                {filterOptions.filter((opt) => opt.key !== null).map((opt) => (
                  <div key={String(opt.key)} className="h-full transition-all"
                    style={{ width: `${total > 0 ? (opt.count / total) * 100 : 0}%`, backgroundColor: opt.color }} />
                ))}
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
                {filterOptions.filter((opt) => opt.key !== null).map((opt) => (
                  <div key={String(opt.key)} className="flex items-center gap-2 rounded-xl bg-slate-50 px-2.5 py-2 dark:bg-slate-700/40">
                    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full" style={{ backgroundColor: `${opt.color}20`, color: opt.color }}>
                      <opt.icon size={13} />
                    </span>
                    <div className="min-w-0">
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-200">{opt.count}</p>
                      <p className="truncate text-[9px] text-slate-400">{opt.label} · {total > 0 ? Math.round((opt.count / total) * 100) : 0}%</p>
                    </div>
                  </div>
                ))}
              </div>
