...
import { Activity, Building2, TrendingUp, AlertTriangle, Search, Filter } from "lucide-react";
...
export default function AdminMagangMonitoringPage() {
  ...
  const STATS = [
    { label: "Penempatan Aktif", value: summary.totalPenempatan, suffix: " siswa", icon: Activity },
    { label: "Tempat PKL Digunakan", value: summary.totalTempat, suffix: " tempat", icon: Building2 },
  ...
  return (
    <div className="space-y-5 p-1">
      <div className="relative overflow-hidden rounded-2xl p-6" style={{ background: "#0082FB" }}>
        ...
        <div className="relative flex items-center gap-3 sm:gap-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-white/70">Pantau progres kehadiran seluruh siswa PKL yang sedang aktif</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        ...
        <MonitoringTable loading={loading} rows={filtered} showPembimbing />
      </div>
    </div>
  );
}
