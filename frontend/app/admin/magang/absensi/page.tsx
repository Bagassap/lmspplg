...
import {
  ...
  ArrowRight, ChevronLeft, ChevronRight, ChevronDown, Eye, X,
...
import { STATUS_CFG, PULANG_CFG, WALLET_GRADIENTS, WALLET_ON_TEXT, MONTH_NAMES, RANGE_MODE_CARDS, reportCardFg, todayJakarta, formatTgl, formatTglSlash, avatarColor } from "@/components/absensi-harian/shared";
import type { SiswaAbsensi, StatusAbsensi, FilterAbsensi, RekapTempat, TempatMagang } from "@/components/absensi-magang/types";

function KirimPengingatCard({ tanggal, siswaList, bold }: { tanggal: string; siswaList: SiswaAbsensi[]; bold?: boolean }) {
...
function KehadiranSlimCard({ hadirPct, tempatNama, onClick }: { hadirPct: number; tempatNama?: string; onClick: () => void }) {
  return (
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-[11px] font-semibold text-slate-400 dark:text-slate-500">
            Kehadiran Hari Ini{tempatNama ? ` · ${tempatNama}` : ""}
          </p>
...
function MobileSiswaCard({
  s, isPulangView, tempatMagangId, tanggal, onStatusUpdated, onOpenDokumen,
}: {
  s: SiswaAbsensi; isPulangView: boolean;
  tempatMagangId?: string; tanggal?: string; onStatusUpdated?: () => void;
  onOpenDokumen: (s: SiswaAbsensi, source: "hadir" | "pulang") => void;
}) {
  const toast = useToast();
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const canEdit = !!tempatMagangId && !!tanggal && !isPulangView;
  ...
  async function saveStatus(status: StatusAbsensi) {
    if (!tempatMagangId || !tanggal) return;
    setSaving(true);
    try {
      const res = await fetch("/api/magang/absensi", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ tempatMagangId, tanggal, absensi: [{ siswaId: s.siswaId, status }] }),
      });
      if (res.ok) {
...
export default function AdminMagangAbsensiPage() {
  ...
  const [tempatPickerOpen, setTempatPickerOpen] = useState(false);
      ...
      <div className="relative -m-4 lg:hidden" style={{ background: "#0082FB" }}>
        ...
        <div className="space-y-4 rounded-t-[28px] bg-[#F1F5F8] p-4 dark:bg-[#1C2B33]">
          ...
          <div className="flex items-center gap-2 rounded-3xl bg-white p-2.5 shadow-[0_2px_8px_rgba(0,0,0,0.06)] dark:bg-[#1C2B33]">
            <button type="button" onClick={() => setTempatPickerOpen(true)}
              ...
              <span className="truncate text-sm font-bold text-slate-700 dark:text-slate-200">
                {selected ? `${selected.tempatMagang.namaTempat} · ${hadirPct}% hadir` : "Pilih tempat"}
              </span>
          ...
          <KehadiranSlimCard hadirPct={hadirPct} tempatNama={selected?.tempatMagang.namaTempat} onClick={() => setStatusPageOpen(true)} />

          <div className="grid grid-cols-2 gap-2.5">
            <FlatStatTile icon={ClipboardCheck} value={`${sudahAbsen}/${total}`} label="Progres Absen"
              bg="#0064E0" fg="#FFFFFF" />
            <KirimPengingatCard tanggal={tanggal} siswaList={rekapAll.flatMap((r) => r.siswa)} bold />
          </div>
          ...
          {tempatPickerOpen && (
            <div className="fixed inset-0 z-[60] flex items-end justify-center">
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                onClick={() => setTempatPickerOpen(false)} />
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                ...
                <div className="flex items-center justify-between px-5 pt-5">
                  <h3 className="text-base font-extrabold text-slate-800 dark:text-white">Pilih Tempat PKL</h3>
                  <button type="button" onClick={() => setTempatPickerOpen(false)}
                ...
                <div className="mt-2 overflow-y-auto px-3 pb-5">
                  {tempatList.map((t) => {
                    const s = tempatStat(t);
                    const isSelected = t.id === selectedId;
                    return (
                      <button key={t.id} type="button"
                        onClick={() => { setSelectedId(t.id); setTempatPickerOpen(false); }}
                        className="flex w-full items-center justify-between rounded-2xl px-3 py-3 text-left transition-colors"
                        style={isSelected ? { background: "#EAF3FF" } : undefined}>
                        <div className="min-w-0">
                          <p className={`truncate text-sm font-bold ${isSelected ? "" : "text-slate-800 dark:text-slate-100"}`}
                            style={isSelected ? { color: "#0082FB" } : undefined}>{t.namaTempat}</p>
                          <p className="truncate text-[11px] text-slate-400">{s.hd}/{s.tt} hadir · Izin {s.iz} · Sakit {s.sk} · Alpa {s.al}</p>
                        </div>
                        <span className="shrink-0 rounded-lg px-2 py-1 text-xs font-bold"
                          style={{ color: isSelected ? "#0082FB" : "#64748B", background: isSelected ? "#DCEBFF" : "#F1F5F9" }}>
                          {s.pct}%
                        </span>
                      </button>
                    );
                  ...
                  {loading ? (
                    ...
                    <div className="flex flex-col items-center gap-2 py-12 text-center">
                      ...
                      <p className="text-xs font-medium text-slate-400 dark:text-slate-500">
                        {siswaList.length === 0 ? "Belum ada siswa PKL di tempat ini" : "Tidak ada siswa dengan status ini"}
                      </p>
                    </div>
                  ) : (
                    <div className="divide-y divide-slate-100 px-4 dark:divide-slate-700/40">
                      {pagedSiswa.map((s) => (
                        <MobileSiswaCard key={s.siswaId} s={s} isPulangView={activeFilter === "PULANG"}
                          tempatMagangId={selectedId} tanggal={tanggal} onStatusUpdated={loadRekap} onOpenDokumen={(sw, source) => { setDokumenSiswa(sw); setDokumenSource(source); }} />
                      ))}
                    </div>
                  )}
                </div>
