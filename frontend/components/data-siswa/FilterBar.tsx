...
export function FilterBar({
  ...
  return (
    <div className="relative flex flex-col p-4 sm:p-5">
      <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
          ...
          <div className="flex w-full items-center gap-2">
            ...
            {compact && (
              <div className="flex shrink-0 items-center gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-700/50">
                {GENDER_PILLS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      style={active ? { backgroundColor: opt.color, color: "#fff" } : { color: "#94a3b8" }}
                    >
                      <OptIcon size={15} />
                    </button>
                  );
      ...
      {compact ? (
        !loading && !isFiltered && (
          <p className="relative mt-3 text-[11px] font-medium text-slate-400 dark:text-slate-500">
            Menampilkan {totalCount} siswa
          </p>
        )
      ) : (
