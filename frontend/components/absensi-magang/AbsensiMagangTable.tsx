...
import { STATUS_CFG, PULANG_CFG, avatarColor, parseLokasi } from "@/components/absensi-harian/shared";
...
export function AbsensiMagangTable({
  ...
  onOpenDokumen, tempatMagangId, tanggal, onStatusUpdated,
}: Props) {
  ...
  const canEdit = !!tempatMagangId && !!tanggal;
  ...
  async function saveStatus(siswaId: string, status: StatusAbsensi) {
    ...
    try {
      const res = await fetch("/api/magang/absensi", {
        ...
        body: JSON.stringify({ tempatMagangId, tanggal, absensi: [{ siswaId, status }] }),
      });
      if (res.ok) {
  ...
  if (!hasSiswa) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm font-medium text-slate-400 dark:text-slate-500">Belum ada siswa PKL di tempat ini</p>
      </div>
    );
  }
